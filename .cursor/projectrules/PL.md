...
' ==========================================
' 🏗️ XY相遇派完整系统 - 综合类图设计 v7.1 (数据分析增强版)
' 🔧 v7.0 重大重构：
...
' 7. 消息表明确分片策略
' 
' 🔧 v7.1 数据分析增强（2025-10-13）：
' 1. UserBehavior表增强：24字段（+16），支持会话/转化/深度分析
' 2. SearchHistory表优化：15字段（+10），增加点击分析/满意度判断
' 3. HotSearch表扩展：23字段（+15），增加趋势/排名/相关词推荐
' 4. UserPreference表完善：28字段（+15），整合筛选/通知/隐私设置
' 5. City表重构：24字段（+15），增加POLYGON边界/多维统计
' 6. 新增分区策略：UserBehavior/SearchHistory按日期分区
' 7. 新增采样策略：高频行为10%采样存储
' 8. 新增归档策略：7天热/30天温/ClickHouse冷
' ==========================================
' 设计理念：生产级数据库设计，高性能、高可用、高安全、数据驱动
' 核心模块：登录认证、游戏服务、生活服务、活动组局、评价系统、发现页面、个人主页、消息通信、行为分析
' 表数量：60张表（新增7张核心表）
' 参考文档：发现页面模块架构设计文档v1.6、个人主页模块架构设计文档v2.0、个人信息编辑模块架构设计文档v1.0、消息模块架构设计文档v1.0、探店APP登录模块架构设计、探店APP登录模块完整功能矩阵、AAA_TECH_STACK_REQUIREMENTS.md
' ==========================================
...
' **v7.1 数据分析增强（2025-10-13）：** 🆕
' 1. 行为追踪升级 ✅
'    - UserBehavior: 8字段 → 24字段
'    - 新增：session_id会话追踪、scroll_depth滚动深度
'    - 新增：conversion转化分析、device_id设备分析
'    - 新增：date_partition分区键、异步批量写入
'    - 优化：7天热数据 + 30天归档 + ClickHouse冷数据
' 
' 2. 搜索分析升级 ✅
'    - SearchHistory: 5字段 → 15字段
'    - 新增：first_click_position点击位置、is_satisfied满意度
'    - 新增：keyword_normalized标准化、search_scope搜索范围
'    - 新增：total_click_count多次点击、search_duration停留时长
'    - 优化：按日期分区、Redis缓存最近10次搜索
' 
' 3. 热搜系统升级 ✅
'    - HotSearch: 8字段 → 23字段
'    - 新增：trend_score趋势分数、rank_change排名变化
'    - 新增：click_through_rate点击率、conversion_rate转化率
'    - 新增：related_keywords相关词、is_rising热度上升
'    - 优化：Redis Sorted Set实时榜单、每5分钟更新
' 
' 4. 用户偏好升级 ✅
'    - UserPreference: 13字段 → 28字段
'    - 新增：filter_verified_only认证筛选、auto_location自动定位
'    - 新增：notification_categories通知分类、隐私设置字段
'    - 新增：last_location位置记忆（POINT类型）
'    - 优化：Redis缓存1小时、SPATIAL INDEX空间索引
' 
' 5. 城市数据升级 ✅
'    - City: 9字段 → 24字段
'    - 新增：center_location中心点、city_area城市边界（POLYGON）
'    - 新增：content_count/activity_count/service_count多维统计
'    - 新增：growth_rate增长率、is_open开通状态
'    - 优化：SPATIAL INDEX空间索引、Redis永久缓存
' 
' **v7.1 设计原则：**
' • 数据驱动：完整的行为追踪 → 转化分析 → 偏好学习
' • 性能优先：分区表 + 异步写入 + 采样存储 + 冷热分离
' • 智能推荐：会话关联 + 转化漏斗 + 协同过滤
' • 空间优化：POINT/POLYGON类型 + SPATIAL INDEX
' • 隐私合规：游客支持 + 数据脱敏 + 定期清理
' ==========================================
...
class SearchHistory {
    ...
    ' 搜索历史表（搜索行为分析）
    ' id: 搜索记录ID
    ' user_id: 搜索用户ID（支持游客NULL）
    ' session_id: 会话ID（关联UserBehavior）
    ' keyword: 搜索关键词（原始输入）
    ' keyword_normalized: 关键词标准化（去空格/小写/繁简转换，用于去重统计）
    ' search_type: 搜索类型（1=综合,2=用户,3=内容,4=话题,5=服务,6=活动）
    ' search_scope: 搜索范围（1=全站,2=同城,3=附近,4=关注）
    ' filter_params: 筛选参数（JSON，如价格/距离/评分等）
    ' result_count: 搜索结果数量
    ' first_click_position: 首次点击位置（第几个结果，用于分析排序质量）
    ' first_click_target_id: 首次点击目标ID
    ' total_click_count: 总点击次数（同一次搜索的多次点击）
    ' search_duration: 搜索停留时长（秒，用于分析满意度）
    ' is_satisfied: 是否满意（基于点击/停留判断）
    ' device_id: 设备标识
    ' device_type: 设备类型
    ' ip_address: IP地址
    ' city_id: 城市ID
    ...
    ' 🔍 INDEX idx_keyword (keyword_normalized, created_at)
    ' 🔍 INDEX idx_session (session_id)
    ' 🔍 INDEX idx_partition (date_partition)
}
...
class UserPreference {
    ...
    ' id: 偏好记录ID
    ' user_id: 用户ID（唯一索引）
    ' preference_category: 偏好分类（1=首页筛选,2=发现筛选,3=通知设置,4=隐私设置）
    ' filter_game_types: 游戏类型筛选（JSON数组字符串，如"[1,2,3]"）
    ' filter_service_types: 服务类型筛选（JSON数组）
    ' filter_activity_types: 活动类型筛选（JSON数组）
    ' filter_price_min: 最低价格（分）
    ' filter_price_max: 最高价格（分）
    ' filter_distance_km: 最大距离（公里，0=不限）
    ' filter_gender: 性别筛选（0=不限,1=仅男,2=仅女）
    ' filter_age_min: 最小年龄
    ' filter_age_max: 最大年龄
    ' filter_online_only: 仅显示在线（0=否,1=是）
    ' filter_verified_only: 仅显示认证用户（0=否,1=是）
    ' filter_rating_min: 最低评分（1.0-5.0）
    ' sort_by: 排序字段（recommend/distance/price/rating/hot）
    ' sort_order: 排序方向（asc/desc）
    ' auto_location: 自动定位（自动使用当前位置筛选）
    ' last_city_id: 最后选择的城市ID
    ' last_location: 最后位置（POINT类型，用于距离计算）
    ' notification_push_enabled: 推送总开关
    ' notification_categories: 推送分类（JSON数组，如"[1,2,3]"）
    ' privacy_who_can_message: 谁可以发消息（0=所有人,1=我关注的,2=互相关注,3=不允许）
    ' privacy_who_can_view_profile: 谁可以看资料（0=所有人,1=我关注的,2=互相关注）
    ' privacy_show_online_status: 显示在线状态（0=隐身,1=显示）
    ' use_count: 使用次数（筛选应用次数统计）
...
class UserBehavior {
    ...
    ' 用户行为追踪表（数据分析核心表）
    ' id: 行为记录ID（雪花ID）
    ' user_id: 用户ID（索引字段，支持游客NULL）
    ' session_id: 会话ID（UUID，用于关联同一次访问的多个行为）
    ' behavior_type: 行为类型（1=浏览,2=点击,3=搜索,4=筛选,5=停留,6=分享,7=收藏,8=下单,9=支付,10=退出）
    ' behavior_action: 行为动作（view_content/click_button/search_keyword/filter_price/share_wechat等）
    ' target_type: 目标类型（1=用户详情,2=内容,3=活动,4=游戏服务,5=生活服务,6=搜索结果,7=话题）
    ' target_id: 目标对象ID（关联具体业务表）
    ' target_title: 目标标题（冗余存储，避免关联查询）
    ' target_user_id: 目标所属用户ID（如查看某人主页，冗余优化）
    ' duration_seconds: 停留时长（秒，页面停留/视频播放时长）
    ' scroll_depth: 滚动深度（百分比，0-100，用于分析内容吸引力）
    ' source_page: 来源页面（/home/discover/profile等）
    ' source_module: 来源模块（recommend_flow/hot_topic/nearby_service等）
    ' referrer: 引荐来源（上一页URL或分享来源）
    ' is_conversion: 是否转化（浏览→下单/点击→关注/搜索→访问等）
    ' conversion_type: 转化类型（1=下单,2=关注,3=收藏,4=分享,5=评论,6=点赞）
    ' conversion_value: 转化价值（订单金额/分享次数等，分）
    ' device_id: 设备标识（用于设备级别行为分析）
    ' device_type: 设备类型（iOS/Android/Web）
    ' ip_address: IP地址（地域分析）
    ' city_id: 城市ID（关联City表，冗余优化）
    ' network_type: 网络类型（WiFi/4G/5G，用于分析网络对行为的影响）
    ' app_version: 应用版本号（版本迭代分析）
    ' created_at: 行为时间（精确到秒）
    ' date_partition: 日期分区键（YYYY-MM-DD，用于分区表优化）
    ' 
    ' 🔧 性能优化策略：
    ' 1. 按日期分区（PARTITION BY RANGE (TO_DAYS(date_partition))）
    ' 2. 热数据7天（当前分区），温数据30天，冷数据归档
    ' 3. 异步写入（消息队列批量入库，减少实时写压力）
    ' 4. 定时汇总到分析表（每小时/每日汇总统计）
    ' 5. 采样存储（高频行为如滚动事件，10%采样存储）
    ' 
    ' 🔧 索引设计：
    ' 🔍 PRIMARY KEY (id)
    ...
    ' 🔍 INDEX idx_conversion (is_conversion, conversion_type, created_at)
    ' 🔍 INDEX idx_partition (date_partition) -- 分区裁剪优化
    ' 
    ' 🔧 数据清理策略：
    ' - 7天内数据：保留在MySQL分区表（高速查询）
    ' - 8-30天数据：保留在MySQL（归档分区）
    ' - 30天以上：导出到ClickHouse（OLAP分析）或对象存储
    ' - 定时任务每日凌晨2点执行归档
}
...
class HotSearch {
    ...
    ' id: 热搜记录ID
    ' keyword: 搜索关键词（显示用）
    ' keyword_normalized: 标准化关键词（去重统计用）
    ' search_count_today: 今日搜索次数（每日0点重置）
    ' search_count_week: 本周搜索次数（每周一重置）
    ' search_count_total: 历史总搜索次数
    ' click_count: 点击次数（搜索后点击结果）
    ' click_through_rate: 点击率（click_count / search_count）
    ' conversion_count: 转化次数（点击后产生行为）
    ' conversion_rate: 转化率（conversion_count / search_count）
    ' avg_result_count: 平均结果数量（用于判断搜索质量）
    ' category: 关键词分类（1=用户名,2=内容,3=游戏,4=服务,5=活动,6=话题,7=地点,8=综合）
    ' heat_score: 热度分数（综合计算，用于排序）
    ' trend_score: 趋势分数（热度增长率，识别爆发词）
    ' rank_position: 当前排名（1-50，榜单位置）
    ' rank_change: 排名变化（相比昨日，正数=上升，负数=下降）
    ' is_rising: 是否热度上升（标识🔥）
    ' related_keywords: 相关搜索词（JSON数组，如"["王者荣耀","陪玩"]"）
    ' icon_url: 图标URL（热词配图）
    ' link_url: 跳转链接（热词专题页）
    ' status: 状态（0=下线,1=正常,2=推荐,3=置顶,4=屏蔽）
    ...
    ' 
    ' 🔧 索引设计：
    ' 🔍 UNIQUE INDEX uk_keyword (keyword_normalized)
    ' 🔍 INDEX idx_heat (heat_score DESC, rank_position)
    ' 🔍 INDEX idx_category (category, status, heat_score DESC)
    ' 🔍 INDEX idx_trend (is_rising, trend_score DESC)
}
...
class City {
    ...
    ' 城市数据表（地域服务核心）
    ' id: 城市ID（主键）
    ' code: 城市代码（国标GB/T 2260，如"440300"）
    ' name: 城市名称（如"深圳市"）
    ' province: 所属省份（如"广东省"）
    ' province_code: 省份代码（如"440000"）
    ' level: 城市级别（1=一线,2=新一线,3=二线,4=三线,5=其他）
    ' center_location: 城市中心点（POINT类型，用于距离计算）
    ' city_area: 城市边界（POLYGON类型，用于判断点是否在城市内）
    ' user_count: 用户数量（定时统计）
    ' content_count: 内容数量（动态+活动）
    ' activity_count: 活动数量
    ' service_count: 服务数量（游戏+生活）
    ' hot_score: 热度分数（综合用户数/内容数/活跃度）
    ' growth_rate: 增长率（用户增长率，百分比）
    ' pinyin: 全拼（shenzhen，用于搜索）
    ' short_pinyin: 首字母（sz，用于快速索引A-Z）
    ' area_code: 区号（如"0755"）
    ' timezone: 时区（Asia/Shanghai）
    ' weather_code: 天气代码（接入天气API用）
    ' sort_order: 排序顺序（热门城市优先显示）
    ' is_hot: 是否热门城市（前20城市）
    ' is_open: 是否开通服务（灰度开放城市）
    ' status: 状态（0=未开通,1=已开通,2=维护中）
    ' created_at: 创建时间
    ' updated_at: 更新时间
    ' 
    ' 🔧 空间索引优化：
    ...
    ' 🔍 INDEX idx_short (short_pinyin, sort_order)
    ' 🔍 SPATIAL INDEX idx_center_location (center_location)
}
...
note right of UserPreference
  **用户偏好系统（个性化核心）**
  • 筛选偏好：游戏/服务/活动类型、价格距离范围
  • 用户筛选：性别、年龄、在线状态、认证筛选
  • 排序偏好：推荐/距离/价格/评分/热度
  • 位置偏好：自动定位、最后城市、POINT空间索引
  • 通知偏好：推送开关、分类设置
  • 隐私设置：谁可以发消息、查看资料、在线状态
  • 使用统计：应用次数、最后使用时间
  • Redis缓存：1小时TTL，高频读取优化
end note

note as UserBehaviorSystem
  **🔍 用户行为分析体系（数据驱动核心）**
  
  **1. 行为追踪层（UserBehavior表）**
  ├─ 会话维度：session_id关联单次访问的多个行为
  ├─ 行为类型：浏览/点击/搜索/筛选/停留/分享/收藏/下单/支付/退出
  ├─ 目标追踪：target_type + target_id + target_title（冗余优化）
  ├─ 深度分析：停留时长、滚动深度、来源模块
  ├─ 转化分析：is_conversion + conversion_type + conversion_value
  ├─ 设备分析：device_id + device_type + network_type
  ├─ 地域分析：ip_address + city_id
  └─ 版本分析：app_version（迭代优化依据）
  
  **2. 搜索分析层（SearchHistory表）**
  ├─ 搜索质量：result_count + first_click_position
  ├─ 满意度：search_duration + total_click_count + is_satisfied
  ├─ 关键词：keyword_normalized（标准化去重）
  ├─ 搜索范围：全站/同城/附近/关注
  ├─ 筛选参数：filter_params（JSON存储）
  └─ 智能推荐：基于历史优化排序
  
  **3. 热搜榜单层（HotSearch表）**
  ├─ 多维统计：今日/本周/历史搜索次数
  ├─ 转化指标：点击率 + 转化率
  ├─ 趋势识别：trend_score + rank_change + is_rising
  ├─ 排名系统：rank_position（1-50）
  ├─ 相关推荐：related_keywords（JSON数组）
  └─ Redis存储：ZADD hot_search:score（实时榜单）
  
  **4. 偏好学习层（UserPreference表）**
  ├─ 筛选记忆：价格/距离/性别/年龄/评分
  ├─ 位置记忆：last_location（POINT类型）
  ├─ 排序偏好：sort_by + sort_order
  ├─ 隐私设置：谁可以发消息/查看资料
  └─ 使用频率：use_count统计
  
  **5. 性能优化策略**
  ├─ 数据分区：UserBehavior按日期分区（7天热/30天温/归档冷）
  ├─ 异步写入：消息队列批量入库（降低实时压力）
  ├─ 采样存储：高频事件10%采样（如滚动事件）
  ├─ 定时汇总：每小时汇总到分析表
  ├─ 冷数据归档：30天以上导出ClickHouse/对象存储
  └─ Redis缓存：热门搜索/用户偏好/最近行为
  
  **6. 数据分析应用**
  ├─ 个性化推荐：基于行为历史 + 协同过滤
  ├─ 搜索优化：基于点击位置调整排序权重
  ├─ 转化漏斗：浏览→点击→下单→支付链路分析
  ├─ 留存分析：用户行为频次 + 活跃度
  ├─ 内容质量：停留时长 + 滚动深度评估
  └─ 城市运营：地域用户行为差异分析
  
  **7. 隐私合规**
  ├─ 游客行为：user_id可为NULL（支持未登录追踪）
  ├─ 数据脱敏：IP地址仅保留城市级别
  ├─ 数据保留：30天后归档，90天后删除明细
  └─ 用户删除：账号注销时清除所有行为数据
end note
...
note as DataDesignPrinciples
  ...
  **索引建议(行为分析模块)** 🆕
  🔍 UserBehavior: (user_id, behavior_type, created_at) - 用户行为查询
  🔍 UserBehavior: (target_type, target_id, created_at) - 目标行为查询
  🔍 UserBehavior: (session_id, created_at) - 会话行为追踪
  🔍 UserBehavior: (is_conversion, conversion_type, created_at) - 转化分析
  🔍 UserBehavior: (date_partition) - 分区裁剪优化
  🔍 UserBehavior: (device_id, created_at) - 设备行为分析
  🔍 SearchHistory: (user_id, created_at DESC) - 用户搜索历史
  🔍 SearchHistory: (keyword_normalized, created_at) - 关键词统计
  🔍 SearchHistory: (session_id) - 会话搜索关联
  🔍 SearchHistory: (date_partition) - 分区优化
  🔍 HotSearch: (keyword_normalized) UNIQUE - 关键词唯一
  🔍 HotSearch: (heat_score DESC, rank_position) - 热度排序
  🔍 HotSearch: (category, status, heat_score DESC) - 分类热搜
  🔍 HotSearch: (is_rising, trend_score DESC) - 趋势榜单
  🔍 UserPreference: (user_id) UNIQUE - 用户偏好唯一
  🔍 UserPreference: SPATIAL INDEX (last_location) - 位置记忆
  🔍 City: (code) UNIQUE - 城市代码唯一
  🔍 City: (is_hot, hot_score DESC) - 热门城市
  🔍 City: (pinyin) - 拼音搜索
  🔍 City: (short_pinyin, sort_order) - 首字母索引
  🔍 City: SPATIAL INDEX (center_location) - 城市中心点
  🔍 City: SPATIAL INDEX (city_area) - 城市边界
  ...
  **性能优化(行为分析模块)** 🆕
  📊 UserBehavior: 按日期分区(PARTITION BY RANGE(TO_DAYS(date_partition)))
  📊 UserBehavior: 异步批量写入(消息队列10秒批次,单次500条)
  📊 UserBehavior: 高频采样(滚动事件10%采样,减少数据量)
  📊 UserBehavior: 7天热数据+30天温数据+归档冷数据(ClickHouse/OSS)
  📊 UserBehavior: 定时汇总(每小时汇总到BehaviorSummary分析表)
  📊 SearchHistory: 按日期分区(30天热数据,历史归档)
  📊 SearchHistory: Redis缓存最近10次搜索(快速展示搜索历史)
  📊 SearchHistory: 关键词标准化(keyword_normalized去重统计)
  📊 HotSearch: Redis Sorted Set实时榜单(ZADD hot_search:score)
  📊 HotSearch: 每5分钟更新热度分数(定时任务计算)
  📊 HotSearch: 每小时计算排名变化(rank_change字段)
  📊 UserPreference: Redis缓存(preference:user:{id}, TTL 1小时)
  📊 UserPreference: 更新时清除缓存(保证数据一致性)
  📊 City: Redis永久缓存全量数据(city:list, 启动加载)
  📊 City: 空间索引优化(SPATIAL INDEX, ST_Contains判断城市边界)
  📊 City: 定时统计(每日凌晨统计user_count/content_count等)
end note
